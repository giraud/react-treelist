...
class Body extends Component {
    ...
    makeRowsRecursive(row, level, metadata, columns, idField, parentIdField, rowClass) {
        ...
        rows.push(
            createRow(
                ...
                idField,
                canExpand,
                this.state.expandedRows.indexOf(row[idField]) > -1,
                this.handleExpandToggle,
                this.props.canSelect ? this.handleSelectRow : () => {},
                row[idField] === this.state.selectedRow,
                rowClass,
            ),
        )
    ...
    render() {
        const { columns, data, metadata, idField, parentIdField, width, height, expandAll, rowClass } = this.props

        const rows = this.makeRows(data, metadata, columns, idField, parentIdField, expandAll, rowClass)
        ...
        return (
            <div className="tgrid-body-wrapper" onScroll={this.onScroll} style={{ height: height }}>
